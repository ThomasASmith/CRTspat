...
#' @param analysis list: analysis object generated by Analyse_CRT using method equal to one of 'LR','CRE','SPDE','SPCRE'
#' @param showDistance logical: determining whether smoothed raster of the distance from nearest discordant location is shown
#' @param showProportion logical: determining whether a smoothed raster of the fitted proportion is shown
...
# combinations that are too cluttered or overprinted

  if(is.null(analysis)) {
    showdistance=FALSE
    showProportion=FALSE
  } else if(showProportion) {
    showdistance=FALSE
...
# the coordinates should all be Cartesian at this point but long and lat appear later (??)
  ...
  for(i in 1:totalClusters){
    ...
    if(showClusterBoundaries){
      g = g + get_Polygon(polygon_type='clusterboundaries',i=i,totalClusters=totalClusters,
                          d=d,x=long,y=lat,cpalette=cpalette)
    }
    if(colourClusters){
  ...
  if(showArms){
    g = g + ggplot2::scale_fill_manual("",values=cpalette,labels=c("Control", "Intervention"))
    g= g + ggplot2::coord_equal()
  }

  ############ raster images derived from inla analysis #############
  if(showProportion){
    g=g + ggplot2::geom_tile(data=analysis$inlaMesh$prediction,
                             aes(x=x,y=y, fill = proportion), alpha = 0.5)
    g=g + ggplot2::scale_fill_gradient(name = "Proportion",low = "blue", high = "orange")
  }
  if(showDistance){
    g=g + ggplot2::geom_tile(data=analysis$inlaMesh$prediction,
                             aes(x=x,y=y, fill = nearestDiscord), alpha = 0.5)
  ...
  if(showContamination){
    ...
    analysis$inlaMesh$prediction$contaminated = ifelse(dplyr::between(analysis$inlaMesh$prediction$nearestDiscord,
                                range[1], range[2]),TRUE,FALSE)
    g=g + ggplot2::geom_raster(data=analysis$inlaMesh$prediction[analysis$inlaMesh$prediction$contaminated,],
...
get_Polygon = function(polygon_type,i, totalClusters=totalClusters, d=d,x=long,y=lat,cpalette=cpalette){
    ...
    polygon <- ggplot2::geom_polygon(data=d[as.numeric(d$id)==i,],
                              aes(x=long, y=lat), colour="gray", fill="white",size =0.5)
