...
Plot_CRTmap <- function(trial = trial, showLocations = TRUE, showClusterBoundaries = TRUE, showClusterLabels = TRUE,
                        colourClusters = TRUE, showArms = TRUE, analysis = NULL, showDistance = FALSE, showProportion = FALSE, showContamination = FALSE,
  ...
  if (showArms) {
    ...
    if (is.null(cpalette)) {
      cpalette <- c("#F8766D", "#00BFC4", "#C77CFF", "#7CAE00")
    }
  }
...
Plot_Contamination <- function(analysis) {
  ...
  g <- ggplot2::ggplot(data = analysis$contamination$data,aes(x = d, y = p))
