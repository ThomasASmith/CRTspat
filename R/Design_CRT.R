...
#' @param algo algorithm used to determine cluster boundaries, options are-
...
#'                 ICC=0.10, effect=0.4, pC=0.35, desiredPower = 0.8, algo = "kmeans",
#'                 buffer.width=0.25, h=100, outcome.type ="Dichotomous")
Design_CRT <- function(trial, alpha = 0.05,
                       desiredPower = 0.8, effect, ICC, pC, buffer.width = 0, h, algo = "kmeans",
                       outcome.type = outcome.type) {

    input.parameters <- list(pC = pC, alpha = alpha, desiredPower = desiredPower, h = h,
                             algo = algo, outcome.type =outcome.type, buffer.width = buffer.width, effect = effect, ICC = ICC)

    # Step J: specify or compute cluster boundaries
    CRT <- DefineClusters(trial = trial, h = h, algo = algo, reuseTSP = FALSE)
    ...
    # only if a buffer width > 0 is input)
    CRT <- Specify_CRTbuffer(CRT, bufferWidth = buffer.width)

    trial <- convertCRTtodataframe(CRT)
    CRT$CRT.design.full <- describeTrial(trial, input.parameters = input.parameters)
    if (buffer.width > 0) {

        CRT$CRT.design.core <- describeTrial(trial = trial[trial$buffer == FALSE, ], input.parameters = input.parameters)
    }
    CRT$input.parameters <- input.parameters
    return(CRT)
}

# Characteristics of a trial design. The input is a data frame
describeTrial <- function(trial, input.parameters = NULL) {

    # set the class to data.frame (removing the descriptors) so that the function works both for data.frame and CRT input
    ...
    if (!is.null(trial$arm)){
        ...
        if (!is.null(input.parameters)){
            ...
            CRT.power <- CRT.power(locations = locations,
                                ICC = input.parameters$ICC,
                                pC = input.parameters$pC,
                                effect = input.parameters$effect,
                                outcome.type = input.parameters$outcome.type,
                                alpha = input.parameters$alpha,
                                desiredPower = input.parameters$desiredPower,
                                mean_h = mean_h,
                                sd_h = sd_h)
...
summary.CRT <- function(object, ...) {
    ...
    rownames(output) <- c(
      ...
      "Buffer width :               ",
      "Locations:                                            ",
      "Available clusters (across both arms)                 ",
      "  Per cluster mean number of locations                ",
      "  Per cluster s.d. number of locations                ",
      "S.D. of distance to nearest discordant location (km): ",
      "Cluster randomization:            ",
      "\nSpecification of Requirements\n-----------------------------",
      "Significance level:               ",
      "Type of Outcome                   ",
      "Expected outcome in control arm:  ",
      "Required effect size:             ",
      "\nPower calculations (ignoring contamination)\n------------------                    ",
      "Intra-cluster correlation: ",
      "Design effect:                         ",
      "Nominal power (%)                      ",
       paste0("Clusters required for power of ",
              object$input.parameters$desiredPower*100, "%:     "),
    ...
    if (output[2,1] == '(x, y)') {
        ...
        area <- pi *
            (as.numeric(coordinate.summary[6,1]) - as.numeric(coordinate.summary[1,1])) *
    ...
    sd2 <- ifelse(is.null(object$CRT.design.full$sd_distance), NA, object$CRT.design.full$sd_distance)
    clustersAvailableFull <- with(object$CRT.design.full, locations/(mean_h * 2))
    ...
    if (!is.null(object$CRT.design.core)) {
      output[4, 2] <- object$CRT.design.full$locations
    ...
    if (!is.null(object$input.parameters)) {
        ...
        if (!is.null(object$input.parameters$buffer.width)){
          if (object$input.parameters$buffer.width > 0) {
              output[3, 1] <- paste0(object$input.parameters$buffer.width, " km.")
          } else {
    ...
    if (is.null(object$input.parameters)) {
        ...
        sufficient <- ifelse(clustersAvailableFull >=
                               object$CRT.design.full$clustersRequired, "Yes", "No")
        ...
        if (is.null(object$CRT.design.core)) {
            ...
            output[15, 1] <- 'Full'
            output[15, 2] <- 'Core'
            clustersAvailableCore <- with(object$CRT.design.core, locations/(mean_h * 2))
            sufficientCore <- ifelse(clustersAvailableCore >=
                                   object$CRT.design.core$clustersRequired, "Yes", "No")
...
# Step A: confidence level
# Step B: power
# Step C: Required effect size
# Step D: ICC, obtained from other studies
# Step E: baseline prevalence
# Step F: buffer width based on postulated contamination range in km, obtained from other studies
# Step G\ coordinates of households in study area
# Step H: proposal for the number of households in each cluster
...
CRT.design.full <- list(locations = locations, mean_h = mean_h,
                  sd_h = sd_h, min_k = min_k, clustersRequired = clustersRequired, DE = DE, power = power)
input.parameters <- list(locations = locations, ICC=ICC, effect= effect, alpha = alpha, outcome.type = outcome.type,
  desiredPower = desiredPower, pC=pC, mean_h=mean_h, sd_h=sd_h)
