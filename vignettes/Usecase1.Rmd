---
title: "Use Case 1: Algorithmic specification of clusters and of buffer zones"
output:
  rmarkdown::html_vignette:
  toc: true
vignette: >
  %\VignetteIndexEntry{Use Case 1: Algorithmic specification of clusters and of buffer zones}
...
After each step, `summary` can be used to provide a description of the output `CRTsp` object and `plot` can be used to output a descriptive plot, or a map of the locations, clusters, arms, buffer zones or other geographically structured analysis results.
...
+ The `specify_clusters` function carries out algorithmic assignment of clusters and outputs a `trial` data frame augmented with the cluster assignments. One of three different algorithms must be selected:
  + `algorithm = "NN"` implements a [nearest neighbour algorithm](https://en.wikipedia.org/wiki/Nearest-neighbor_chain_algorithm) which takes a collection of points as input, and creates clusters of points by repeatedly merging in points to form larger clusters. This is the default option.
  + `algorithm = "TSP"` implements the `repetitive_nn` option of the
  [`TSP` package](https://cran.r-project.org/web/packages/TSP/vignettes/TSP.pdf) for solving the travelling salesman problem. This finds an efficient path through the study locations. Clusters are formed by grouping the required number of locations sequentially along the path. Note that this is not guaranteed to give rise to congruent clusters.
...
+ The `randomizeCRT` function carries out a simple randomization of clusters to arms, and outputs a `trial` data frame augmented with the assignments. (If baseline data are available matched pair randomization is available as an option)
...
##   Per cluster mean number of points                             49.2            
##   Per cluster s.d. number of points                             3.9            
## S.D. of distance to nearest discordant location (km):           0.73          
