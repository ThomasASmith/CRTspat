---
title: "Use Case 4: Estimation of optimal cluster size for a trial with pre-determined buffer width"
output:
  rmarkdown::html_vignette:
  toc: true
vignette: >
  %\VignetteIndexEntry{Use Case 4: Estimation of optimal cluster size for a trial with pre-determined buffer width}
...
##       x -6.28    -2.57    -0.19    -0.05     2.79     5.42   
##       y -3.07    -1.16    -0.43    -0.06     1.09     4.76   
...
CRTscenario <- function(k, CRT, buffer.width) {
    ex <- specify.clusters(CRT, k = k, algo = "kmeans") %>%
        randomizeCRT() %>%
        specify.buffer(buffer.width = buffer.width)
    ...
    powerCalculations <- calculateCRTpower(locations = locations, alpha = 0.05,
        desiredPower = 0.8, effect = 0.3, yC = GEEanalysis$pt.ests$controlY,
        outcome.type = "p", N = GEEanalysis$description$sum.denominators/locations,
        ICC = ICC, k = ex$geom.core$k, sd_h = ex$geom.core$sd_h)
    value <- c(k_full = k, k_core = ex$geom.core$k, clustersRequired = powerCalculations$geom.full$clustersRequired,
        power = powerCalculations$geom.full$power, mean_h = powerCalculations$geom.full$mean_h,
        locations = locations, ICC = ICC)
    return(value)
}
...
```r
total_locations <- example$geom.full$locations
